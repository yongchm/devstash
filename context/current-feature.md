...
- Email Verification on Register — Resend sends a single-use 24h token link after registration; /verify-email validates token and sets emailVerified on the user; Credentials sign-in blocked for unverified users with a typed error code and targeted UI message; GitHub OAuth unaffected; purge-users.ts script added for DB cleanup
- Email Verification Feature Flag — src/lib/flags.ts with flags.emailVerification driven by EMAIL_VERIFICATION_ENABLED env var (defaults true); set to false in .env to bypass verification during development when no Resend domain is configured
